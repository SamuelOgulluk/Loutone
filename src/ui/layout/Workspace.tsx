...
function dropTargetFromPoint(x: number, y: number) {
  for (const node of document.elementsFromPoint(x, y)) {
    if ((node as HTMLElement).closest?.('[data-ws-hide]')) return 'hide' as const
...
type DropHint = SlotId | 'hide' | null
...
export function WorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  const [dropHint, setDropHint] = useState(null as DropHint)
  ...
  const startPanelPointerDrag = useCallback(
    (id: PanelId, e: React.PointerEvent, onIdle?: () => void) => {
      ...
      const onMove = (ev: PointerEvent) => {
        ...
        setDropHint(dropTargetFromPoint(ev.clientX, ev.clientY))
      }
      const onUp = (ev: PointerEvent) => {
        ...
        if (target === 'hide' && id !== 'arrange' && traveled > 6) hidePanel(id)
        else if (target && target !== 'hide' && traveled > 6) movePanelToSlot(id, target)
  ...
  const api = useMemo<WorkspaceApi>(
    ...
    [editMode, fitted, stored.hidden, overflow, hidePanel, showPanel, movePanelToSlot, swapSlots, resetLayout, dropHint, dragPanel, startPanelPointerDrag],
  )
...
export function LayoutDock() {
  ...
  useEffect(() => {
    ...
    const onDown = (e: PointerEvent) => {
      ...
      if (t.closest?.('[data-ws-hide]')) return
        ...
        {panel ? (
          <div className="ws-map-chip-row">
            ...
            {panel !== 'arrange' && (
              <button
                type="button"
                className="ws-map-hide"
                title={`Masquer ${PANEL_META[panel].label}`}
                onClick={(e) => {
                  e.stopPropagation()
                  hidePanel(panel)
                }}
  ...
  return (
    <>
      <button
        ...
        title={crowded ? `${overflow.length} fenêtre${overflow.length > 1 ? 's' : ''} masquée${overflow.length > 1 ? 's' : ''}` : 'Disposer les fenêtres'}
        aria-expanded={open}
        onClick={() => {
          if (open) {
            closeDock()
      ...
      {open &&
        createPortal(
          ...
          >
            ...
            <p className="ws-dock-help">Glisser pour déplacer · × pour masquer</p>
            ...
            <div
              data-ws-hide="1"
              className={`ws-tray-row ${dropHint === 'hide' ? 'is-drop' : ''}`}
            >
              <span className="ws-tray-label">Masquées</span>
              <div className="ws-tray-chips">
                {overflow.length === 0 ? (
                  <span className="ws-tray-empty">Glisser ici ou × sur un panneau</span>
                ) : (
                  overflow.map((id) => (
                    <button
                      key={id}
